...
export const deleteBook = async (req, res) => {
  try {
    ...
    // Handle image deletion from Cloudinary if it exists
    if (book.image) {
    ...
    try {
      const currentFeaturedBooks = await redis.get("featured_books");

      if (currentFeaturedBooks) {
        let featuredBooksArray = JSON.parse(currentFeaturedBooks);

        featuredBooksArray = featuredBooksArray.filter(
          (book) => book._id !== req.params.id
        );

        if (featuredBooksArray.length === 0) {
          await redis.del("featured_books");
        } else {
          await redis.set("featured_books", JSON.stringify(featuredBooksArray));
        }
      } else {
        console.log("No featured books found in Redis.");
      }
    } catch (error) {
      console.error("Error removing book from Redis:", error.message);
    }
